...
exports.RESUME_STYLE_REFERENCE_DEFAULT = void 0;
exports.resolveResumeStyleReference = resolveResumeStyleReference;
/**
 ...
 */
exports.RESUME_STYLE_REFERENCE_DEFAULT = `Zijia Pan
...
function resolveResumeStyleReference(profileField) {
    const fromProfile = profileField?.trim();
    if (fromProfile)
        return fromProfile.slice(0, MAX_REF_CHARS);
    const fromEnv = process.env.REFERENCE_RESUME_TEXT?.trim();
    if (fromEnv)
        return fromEnv.slice(0, MAX_REF_CHARS);
    return exports.RESUME_STYLE_REFERENCE_DEFAULT.slice(0, MAX_REF_CHARS);
}
//# sourceMappingURL=resume-style-reference-default.js.map