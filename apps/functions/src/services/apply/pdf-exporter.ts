...
export function resumeContentToHtml(content: ResumeContent): string {
  ...
  if (content.experience.length > 0) {
    ...
    for (const exp of content.experience) {
      sections.push(
        `<div class="block"><strong>${escapeHtml(exp.title)}</strong> — ${escapeHtml(exp.company)} <span class="date">${escapeHtml(exp.date)}</span></div>`,
      );
  ...
  if (content.education.length > 0) {
    ...
    for (const ed of content.education) {
      ...
      sections.push(
        `<div class="block">${escapeHtml(ed.school)} — ${escapeHtml(ed.degree)}${ed.major ? `, ${escapeHtml(ed.major)}` : ''} <span class="date">${escapeHtml(ed.date)}</span>${gpa}</div>`,
      );
    }
    sections.push('</div>');
  }
  ...
  if (content.skills.length > 0) {
    sections.push(
      `<div class="section"><h2>Skills</h2><p>${content.skills.map((s: string) => escapeHtml(s)).join(', ')}</p></div>`,
    );
  }
